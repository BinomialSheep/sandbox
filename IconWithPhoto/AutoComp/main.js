...
window.addEventListener("DOMContentLoaded", function(){
    ...
    zip.addEventListener("change", function(){
        ...
        3. 郵便番号のフォーマットが正しければ、郵便番号を基に住所をテキストボックスに出力する
        4. 郵便番号がdictZipAddrに存在しなければエラーを出す
        
        FUTURE WORK
        ・全角数字を半角数字に変換する
        ・エラーメッセージ、成功メッセージのCSSを作成する

        なお、特に制約がなければ1, 3を実現する既存の自動更新のライブラリが複数あるため、それらの使用を推奨
        dictZipAddrは郵便番号でソートされているため、二分探索などを用いて高速化可能だが、今の実装でも十分高速
        */
...
// 郵便番号と住所の対応
// ローカルで使うなら、日本郵政からダウンロードして適当に前処理してここに追加
var dictZipAddr = {
    '100-0000' : '東京都千代田区',
    '102-0072' : '東京都千代田区飯田橋'
}
